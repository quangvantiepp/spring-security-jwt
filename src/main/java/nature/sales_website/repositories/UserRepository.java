...
public interface UserRepository extends JpaRepository<User, Long> {
     ...
     @Query(value = "SELECT * FROM users u where u.id= :userId and u.phone_number= :phoneNumber", nativeQuery = true)
     User getUserByIdAndPhoneNumber(Long userId, String phoneNumber);
     @Query(value = "SELECT * FROM users u where u.id= :userId and u.password= :password", nativeQuery = true)
